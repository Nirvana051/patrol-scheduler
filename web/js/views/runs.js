...
function inspCard(i) {
  return `<div class="insp"><div>${i.annot_url ? `<img src="${i.annot_url}" alt="全景" loading="lazy">` : '<div class="video-box">无图</div>'}${i.crop_url ? `<img class="crop" src="${i.crop_url}" alt="裁切" loading="lazy">` : ''}</div>
    <div><div style="display:flex;justify-content:space-between;align-items:center"><div><b>${esc(i.waypoint_name)}</b> <span class="muted small">${i.angle_from}° → ${i.angle_to}° · ${fmt.time(i.created_at)} · ${i.latency_ms} ms</span></div><div><span class="answer ${i.answer}">${esc(ANSWER_TEXT[i.answer] || i.answer)}</span> ${i.effective_passed === 1 ? badge('通过', 'ok') : i.effective_passed === 0 ? badge('不通过', 'bad') : badge('待复核', 'warn')}${i.human_passed != null ? ` <span class="small muted" title="${esc(i.human_note || '')}">人工改判${i.human_passed !== i.passed ? '（与 VLM 不一致）' : ''}</span>` : ''}</div></div>
