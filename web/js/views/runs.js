...
export async function render(root, { store, params }) {
  ...
  async function paint() {
    ...
    el.innerHTML = `
      ...
      <div class="card" style="margin-top:14px"><div class="card-head"><h2>本次执行的事件</h2><a class="btn btn-sm" href="/api/events/export.csv?run_id=${r.id}" target="_blank">⬇ 导出 CSV</a></div><div id="revents">${r.events.slice(-60).reverse().map(e => `<div class="event-row ${e.level}"><span class="t">${fmt.time(e.ts)}</span><span>${badge(e.source === 'cloud' ? '云端' : '系统', e.source === 'cloud' ? 'info' : '')}</span><span class="mono small">${esc(e.type)}</span><span>${esc(e.message)}</span></div>`).join('') || '<div class="muted small">无</div>'}</div></div>`;
    el.querySelectorAll('[data-verdict]').forEach(b => b.onclick = async () => { const v = b.dataset.verdict; const note = v === '' ? '' : (prompt('备注（可空）', '') ?? null); if (note === null) return; try { await api(`/api/inspections/${b.dataset.id}/verdict`, { method: 'PUT', body: { passed: v === '' ? null : v === '1', note } }); toast('已记录人工复核', 'ok'); } catch (e) { toast(e.message, 'bad'); } });
...
function inspCard(i) {
  return `<div class="insp"><div>${i.annot_url ? `<img src="${i.annot_url}" alt="全景" loading="lazy">` : '<div class="video-box">无图</div>'}${i.crop_url ? `<img class="crop" src="${i.crop_url}" alt="裁切" loading="lazy">` : ''}</div>
    <div><div style="display:flex;justify-content:space-between;align-items:center"><div><b>${esc(i.waypoint_name)}</b> <span class="muted small">${i.angle_from}° → ${i.angle_to}° · ${fmt.time(i.created_at)} · ${i.latency_ms} ms${i.capture_pose ? ` · 抓图时机头 ${fmt.deg(i.capture_pose.yaw)}` : ''}</span></div><div><span class="answer ${i.answer}">${esc(ANSWER_TEXT[i.answer] || i.answer)}</span> ${i.effective_passed === 1 ? badge('通过', 'ok') : i.effective_passed === 0 ? badge('不通过', 'bad') : badge('待复核', 'warn')}${i.human_passed != null ? ` <span class="small muted" title="${esc(i.human_note || '')}">人工改判${i.human_passed !== i.passed ? '（与 VLM 不一致）' : ''}</span>` : ''}</div></div>
      <div class="small" style="margin-top:4px"><span class="muted">人工复核：</span><button class="btn btn-xs" data-verdict="1" data-id="${i.id}">判通过</button> <button class="btn btn-xs" data-verdict="0" data-id="${i.id}">判不通过</button>${i.human_passed != null ? ` <button class="btn btn-xs" data-verdict="" data-id="${i.id}">撤销</button>` : ''}</div>
