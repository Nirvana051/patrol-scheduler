...
const GROUPS = [
  ...
  { title: 'VLM 视觉判读', keys: [['VLM_PROVIDER', '提供方', 'select:mock,qwen,openai_compat,anthropic', 'qwen = 阿里通义千问（DashScope 兼容模式）；mock 用于无模型环境'], ['VLM_BASE_URL', 'OpenAI 兼容地址', 'text', 'qwen 留空即用 https://dashscope.aliyuncs.com/compatible-mode/v1；Ollama: http://127.0.0.1:11434/v1'], ['VLM_MODEL', '模型', 'text', 'qwen: qwen3.5-flash（图片被拒就换 qwen3-vl-plus / qwen-vl-max）；Ollama: qwen2.5vl:7b'], ['VLM_API_KEY', 'API Key', 'password', 'qwen 用 DashScope 的 sk-… 密钥'], ['VLM_EXTRA_BODY', '额外请求字段', 'text', 'JSON 对象，合并进请求体。qwen 已自动带 enable_thinking:false；需要高分辨率读图可填 {"vl_high_resolution_images": true}'], ['ANTHROPIC_API_KEY', 'Anthropic API Key', 'password', ''], ['ANTHROPIC_MODEL', 'Anthropic 模型', 'text', '默认 claude-opus-5'], ['VLM_ANTHROPIC_FALLBACKS', '启用服务端 fallbacks', 'select:1,0', ''], ['VLM_SEND_FULL_PANO', '附整张全景', 'select:0,1', '除裁切外再附整图'], ['VLM_TIMEOUT', '超时 (s)', 'number', ''], ['VLM_MOCK_ANSWER', 'mock 回答', 'select:alternate,yes,no,unknown,random,keyword', '']] },
  { title: 'TTS 播报', keys: [['TTS_ENGINE', '引擎', 'select:edge,command,none', 'edge 需联网；none 只推文本让浏览器念'], ['TTS_VOICE', 'edge 声音', 'text', 'zh-CN-XiaoxiaoNeural / zh-CN-YunxiNeural'], ['TTS_COMMAND', 'command 模板', 'text', '{text} {out}，如 espeak-ng -v cmn -w {out} {text}'], ['TTS_SINKS', '汇出', 'text', '逗号分隔：browser,local,http,webhook'], ['TTS_AUDIO_SERVER_URL', '播报服务地址', 'text', '本项目自带 audio_server（部署在机器狗/现场 PC），如 http://192.168.0.122:5566'], ['TTS_AUDIO_SERVER_TOKEN', '播报服务口令', 'password', '与 audio_server --token 一致；不填则不鉴权'], ['TTS_WEBHOOK_URL', 'Webhook', 'text', ''], ['TTS_TIMEOUT', '合成超时 (s)', 'number', '外部 TTS 卡住时最多等这么久，超时记错误、执行继续']] },
  { title: '抓图与执行', keys: [['SNAPSHOT_SOURCE', '抓图源', 'text', 'rtsp（首选，TCP）| hls（8554 被挡时的备选）| synthetic | file:<路径> | lavfi:<filter>'], ['FORWARD_DEG', '机头对应角度 (°)', 'number', '全景图上机器人正前方的列，默认 180 = 图像中心；真机首帧校准'], ['SETTLE_SECONDS', '到点稳定 (s)', 'number', ''], ['LEG_TIMEOUT', '每段超时 (s)', 'number', ''], ['MAX_RETRIES', '段失败重试', 'number', ''], ['ESTOP_ON_EXCEPTION', '执行器异常时急停', 'select:1,0', '示例脚本的做法：出异常先急停再收尾'], ['NOTIFY_WEBHOOK_URL', '通知 Webhook', 'text', '检查不通过 / 执行失败中止时 POST JSON 到这里（IM 机器人等）'], ['SCHEDULE_TICK_SECONDS', '定时器检查间隔 (s)', 'number', '']] },
];
...
export async function render(root, { store }) {
  ...
  <div class="grid grid-2">
    ...
    <div>
      ...
      <div class="card" style="margin-top:14px"><h2>备份 / 迁移</h2>
        <div class="form-inline"><a class="btn btn-sm" href="/api/export" target="_blank">⬇ 导出任务航点 / 任务 / 定时（JSON）</a><label class="btn btn-sm">⬆ 导入 JSON<input type="file" id="imp-file" accept="application/json" hidden></label><label class="small"><input type="checkbox" id="imp-over" checked> 同名覆盖</label></div>
        <div class="form-inline" style="margin-top:8px"><span class="small muted">重建图后重定向：</span><input id="rt-from" placeholder="旧地图名" style="width:220px"><span>→</span><input id="rt-to" placeholder="新地图名（已同步）" style="width:220px"><label class="small">≤<input id="rt-dist" type="number" step="0.5" value="3" style="width:60px"> m</label><button class="btn btn-sm" id="rt-go">重定向</button></div>
        <div id="bk-out" class="help">导入时可在文件里的地图名与当前不同的情况下用「重定向」按 x,y 把任务航点挂到新地图最近的导航航点上。</div></div>
  ...
  root.querySelector('#imp-file').onchange = async () => { const f = root.querySelector('#imp-file').files[0]; if (!f) return; try { const data = JSON.parse(await f.text()); const r = await api('/api/import', { method: 'POST', body: { data, overwrite: root.querySelector('#imp-over').checked } }); root.querySelector('#bk-out').textContent = `导入：任务航点 ${r.task_waypoints}、任务 ${r.tasks}、定时 ${r.schedules}，跳过 ${r.skipped}`; toast('导入完成', 'ok'); } catch (e) { toast(`导入失败：${e.message}`, 'bad', 6000); } };
  root.querySelector('#rt-go').onclick = async (e) => { const from = root.querySelector('#rt-from').value.trim(), to = root.querySelector('#rt-to').value.trim(); if (!from || !to) return toast('填写旧/新地图名', 'warn'); try { const r = await busy(e.currentTarget, () => api('/api/task-waypoints/retarget', { method: 'POST', body: { from_map: from, to_map: to, max_distance: Number(root.querySelector('#rt-dist').value) } })); root.querySelector('#bk-out').innerHTML = `已改 ${r.moved}/${r.total}：` + r.report.map(x => `${esc(x.name)} ${x.old_node ?? '?'}→${x.new_node ?? '（超距 ' + x.distance + ' m，未改）'}`).join('；'); } catch (err) { toast(err.message, 'bad', 6000); } };
