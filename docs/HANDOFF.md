...
- mock 上全流程 + 通宵 104 趟稳定。**真机（Gazebo）上 09-05 夜已跑通完整流程**：四点巡检 4 分钟完成，速度 ≈0.8 m/s，检查 3.6 s/点；演练暴露并修了三件事——T22 机器人间歇性不理会停止指令（现已核实 + 告警）、T23 mock→真机切换后事件撞号丢失（schema v6 按网关唯一）、T24 中途被替换的任务云端不发 `task_started`（改按 `total`/路径比对）。通宵每 10 分钟一趟在跑，结果见 `docs/DEVLOG.md` 最新段。
...
---
最后更新：2026-09-05 21:55（真机演练后，通宵计划运行中）。
