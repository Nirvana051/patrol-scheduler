...
| P1 mock 网关 | 23:00–00:00 | 契约仿真 + 运动学 + 事件流 + 故障注入 + 自测 | ⬜ |
| P2 后端核心 | 00:00–02:00 | DB、SDK 包装、事件监听、状态轮询、规划、执行器、检查流水线、VLM/TTS 适配器、REST | ⬜ |
| P3 前端 | 02:00–04:30 | 七个视图，全景角度编辑器，地图画布，SSE 实时 | ⬜ |
| P4 测试 | 04:30–06:00 | 单测 + 对 mock 的端到端执行 + 无头浏览器截图验收 | ⬜ |
