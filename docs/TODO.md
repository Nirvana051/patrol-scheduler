...
| T30 | **最后一个航点的 yaw 对齐超时 → 云端 `task_failed` 错误码 `0x0000`**（09-08 19:30 之后出现）。底层加入 yaw 角控制后，机器人到位还要原地转到地图记录的朝向；去程要转 63°（转到只差 6° 就被切断）、回程要转 180°（每次转到不同程度）。云端等不到「到达」确认就判失败，且 **`errorCode` 填 0**（云端码表把 0 译作 `SUCCESS / 无错误`，所以事件里显示成「failed:0x0000 SUCCESS（无错误）」，字面自相矛盾）。库里 7 次这类失败**全部**是 `visited = total − 1`，只差最后一个点；机器人其实到了（重定位报 0.13–0.42 m）。代价：每段白等 26–37 s，一趟 123 s 里有 70 s（57%）是它。| 巡检结果不受影响（重试 → 重定位 → 「已在航点 X，无需导航」自愈，run 212–215 全部 completed），但流程很不顺滑，且每段多一条红事件 | **用户已在机器人侧解决（yaw 对齐耗时过长）**。<br>**待回归验证**：下一趟真机执行后确认 ① 不再出现 `task_failed 0x0000` ② 最后一跳耗时回到 1.3–2.4 s（09-07/09-08 成功 21 次时的水平）③ 不再有 `leg_retry` / `relocated`。<br>验证方法：`SELECT ts,type,message FROM events WHERE run_id=<新的 run> ORDER BY id`，看最后一个 `waypoint_reached` 与 `leg_arrived` 之间的间隔 |
...
| T35 | **隔离 venv 更占磁盘**：新 `.venv` 199 MB，老的 85 MB —— 因为不再借用系统的 numpy/Pillow/scipy，自己各存一份 | 磁盘（这台机器 1.7T 空闲，不构成问题；小机器上要留意） | 记录事实，不处置。`.venv.old`（85 MB）在新环境稳定后删掉 |
| T36 | **Node/Electron 版（135 个提交）只存在这台机器上**：`origin/main` = `73b721b`，之后全部未推送。已用分支 `node-electron-v0.5.0` + 标签 `rollback-point-20260909` 双重固定 | 这台机器一坏就没了；也因此 CI 的 windows/macos job 从未跑过 | 要保留就得推送（需要 PAT 或配好 SSH）。回去：`git checkout node-electron-v0.5.0` |
