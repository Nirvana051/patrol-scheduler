...
| 页面 | 图 | 说明 |
|------|----|------|
| 总览 | ![](screenshots/01-dashboard.jpg) | 机器人卡片（定位按 received_at 判断）、云端任务语义字段、初始化三步、最近检查、近 30 天按航点统计、下次定时执行 |
...
| 任务规划 | ![](screenshots/05-tasks.jpg) | 任务列表：执行 / 路线预览 / ⏰ 定时 / 编辑 / 删除 |
| 执行监控（完成） | ![](screenshots/06-run-completed.jpg) | 分段进度（幂等键、下发/到达时刻）、轨迹小地图、检查结果（带范围标注的全景 + 裁切 + 回答 + 播报 + 人工复核）、本次事件（可导出 CSV） |
...
| 设置 | ![](screenshots/09-settings.jpg) | 连接 / VLM / TTS / 抓图 / 通知 / 定时器；机头校准；系统自检；备份 / 迁移；演示场景开关 |
